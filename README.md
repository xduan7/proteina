...
We provide config files to reproduce our results. Here we provide commands to reproduce numbers for designability, and get 100 samples for each of the lengths in [50, 100, 150, 200, 250]. Each of these commands will generate a CSV file summarizing results. All config files mentioned in this section build on the file `configs/experiment_config/inference_base.yaml`. The first step involves setting the variable `ckpt_path` in `inference_base.yaml` to the right directory that contains all of the provided checkpoints.
